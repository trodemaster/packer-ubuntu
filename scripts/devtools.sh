...
sudo apt -y install p7zip-full glances jq 
...
#curl -s https://packages.microsoft.com/config/ubuntu/20.04/prod.list | sudo tee /etc/apt/sources.list.d/microsoft.list
#sudo apt-get update
